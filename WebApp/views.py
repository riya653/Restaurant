...
def services(request):
    ...
    cart_count = CartDb.objects.filter(Username=request.session['Username'])
    x = cart_count.count()
    return render(request,"Service.html",{'categories':categories,'service':service,'x':x})
...
def single_item(request,item_id):
    ...
    cart_count = CartDb.objects.filter(Username=request.session['Username'])
    x = cart_count.count()
    return render(request,"Single_Item.html",{'product':product,'x':x})
...
def user_login(request):
    if request.method=="POST":
        ...
        if SignUpDb.objects.filter(Username=un,Password=pswd).exists():
            ...
            messages.success(request,"Welcome to Restoran")
...
def save_cart(request):
    if request.method=="POST":
        ...
        try:
            x= ProductDb.objects.get(Product_Name=productname)
            img=x.Product_Image
